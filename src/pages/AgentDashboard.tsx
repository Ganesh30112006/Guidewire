...
const AgentDashboard = () => {
  ...
  const handleClaimAction = (claimId: string, action: "Approved" | "Rejected", workerName: string) => {
  ...
  const pendingClaims = (claimReviews ?? []).filter((c) => (claimStatusOverrides[c.id] ?? c.status) === "Pending");
  const approvedClaims = (claimReviews ?? []).filter((c) => (claimStatusOverrides[c.id] ?? c.status) === "Approved");
  const totalProcessed = approvedClaims.reduce((s, c) => s + c.requestedPayout, 0);
  ...
  const getPlanName = (planId: string | null) => {
    ...
    return insurancePlans.find((p) => p.id === planId)?.name ?? planId;
  };

  // Filtered claims
  const filteredClaims = (claimReviews ?? [])
                    ...
                    <div key={claim.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-1.5">
                          <button
                            onClick={() => handleClaimAction(claim.id, "Approved", claim.workerName)}
                            className="rounded-md bg-success/10 p-2 text-success hover:bg-success/20 transition-colors" title="Approve">
                            <CheckCircle className="h-4 w-4" />
                          </button>
                          <button
                            onClick={() => handleClaimAction(claim.id, "Rejected", claim.workerName)}
                        ...
                        <td className="px-4 py-3">
                          {c.status === "Pending" ? (
                            <div className="flex gap-1">
                              <button
                                onClick={() => handleClaimAction(c.id, "Approved", c.workerName)}
                                className="rounded bg-success/10 p-1.5 text-success hover:bg-success/20" title="Approve">
                                <CheckCircle className="h-3.5 w-3.5" />
                              </button>
                              <button
                                onClick={() => handleClaimAction(c.id, "Rejected", c.workerName)}
